...
class NormalizeUnitSphere2(object):
    ...
    def __call__(self, data):
        ...
        noisy_std = torch.std(data['pcl_noisy'])
        clean_std = torch.std(data['pcl_clean'])
        data['noise_std'] = noisy_std - clean_std
        return data
...
class AddNoise(object):
    ...
    def __call__(self, data):
        ...
        # data['pcl_noisy_1'] = data['pcl_clean'] + torch.randn_like(data['pcl_clean']) * (noise_std * 0.1)
        # data['pcl_noisy_2'] = data['pcl_clean'] + torch.randn_like(data['pcl_clean']) * (noise_std * 0.3)
        # data['pcl_noisy_3'] = data['pcl_clean'] + torch.randn_like(data['pcl_clean']) * (noise_std * 0.6)
        data['pcl_noisy'] = data['pcl_clean'] + torch.randn_like(data['pcl_clean']) * noise_std
        # data['noise_std_1'] = noise_std * 0.1
        # data['noise_std_2'] = noise_std * 0.3
        # data['noise_std_3'] = noise_std * 0.6
...
def standard_train_transforms(noise_std_min, noise_std_max, scale_d=0.2, rotate=True, add_noise=True):
    if add_noise:
        transforms = [
            NormalizeUnitSphere(),
